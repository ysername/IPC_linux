...
/**************************************************************************************/
/*! \fn void GetTids(const char connection_path[], uint8_t  components[], uint8_t connectionsToWait,  component_info_t * componentsId)
 *
 *  param[in] 
 ...
 *      is listed in the "components" array, and thus this function, acting as
 *      a way to get the tid of the processes at runtime.
...
/**************************************************************************************/
/*! \fn void PostTid(const char connection_path[], uint8_t  components[], uint8_t connectionsToWait,  component_info_t * componentsId)
 *
 *  param[in] 
 *      -connection_path[]: this is the name of the socket file the "server" side will
 *                          be listen to connections
 *      -component:         an uint8_t containing the "component" id of the calling 
 *                          process.
 *      -serverInfo:        a struct containing the tid and a "component" id of the 
 *                          "server" side.
 *
 *  \par Description:
 *      This function will set a client socket in the function that calls it, waiting 
 *      for a connection to be avaible, in wich will send a message specifying its own
 *      tid and "component" id.
 *  \retval 
 *      None.
 *
 *  \par Limitations/Caveats:
...
int8_t WaitSemaphore(const char named_semaphore[]){
    ...
    if(pSemaphore == SEM_FAILED){
        ...
        if((sem_wait(pSemaphore)) > 0){
            ...
            printf("processes are ready!\n");
            return 0;
        }else{
            printf("error while waiting for semaphore!\n");
            sem_unlink(named_semaphore);
            return -1;
        }
    }
